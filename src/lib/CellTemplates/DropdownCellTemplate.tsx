...
export class DropdownCellTemplate implements CellTemplate<DropdownCell> {
  ...
  /**
   * 更新下拉框单元格
   * @param cell - 兼容的下拉框单元格
   * @param cellToMerge - 不确定兼容的下拉框单元格
   * @returns 兼容的下拉框单元格
   */
  update(
    ...
    // 我使用text属性作为selectedValue属性，因为行为不知道selectedValue属性
    // 反而当我们尝试访问它时会抛出错误。
    // 在合并之前，我们还需要检查传入的值是否在目标值数组中，否则将其设置为undefined。
   ...
   * 处理组合框结束事件
   * @param cell - 兼容的单元格对象
   * @param eventData - 组合框结束事件的数据
   * @returns 修改后的单元格对象和是否启用编辑模式的布尔值
   */
